...
insert funcao (nome,cria_ordem_movimentacao,altera_emb_compra,altera_tipo_rastreabilidade,permite_reenvio) 
select distinct funcao,'0',null,'0','0' from mig_usuarios
where  funcao not in 
...
AND  exists (select 1 from ENTIDADE ee where ee.razao_social = mu.nome or ee.fantasia = mu.login)
...
select distinct e.cod_entidade,'1','111111111-11',null,null,null
FROM ENTIDADE e, mig_usuarios mu
where  
e.razao_social = mu.nome AND
convert(varchar, data_cadastro, 103) = '03/06/2022'--DATA DO CADASTRO 
