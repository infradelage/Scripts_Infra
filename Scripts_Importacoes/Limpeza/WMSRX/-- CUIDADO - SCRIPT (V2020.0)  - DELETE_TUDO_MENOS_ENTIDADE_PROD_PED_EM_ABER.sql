...
-- Não delata os pedidos liberados de entrada e saida
...
--Delete somente as ondas que não estejam já em pedidos gerados
DELETE onda WHERE cod_onda not in (SELECT cod_onda FROM pedido where cod_situacao = 1 and operacao in (1,2))
GO
