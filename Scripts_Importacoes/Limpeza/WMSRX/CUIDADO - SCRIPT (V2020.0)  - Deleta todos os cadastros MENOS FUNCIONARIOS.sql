...
go
DELETE  entidade_operacao_logistica  
where cod_entidade > 3 
AND cod_entidade NOT IN (SELECT COD_ENTIDADE FROM entidade WHERE COD_TIPO = 3)--   DBCC CHECKIDENT(entidade, RESEED, 1)
...
--DELETE AS ENTIDADES PRESERVANDO OPER LOGISTICA, USUARIO E FUNCIONARIOS
DELETE entidade where cod_entidade > 3  and COD_TIPO not in (3) --DBCC CHECKIDENT(entidade, RESEED, 3)  
go
