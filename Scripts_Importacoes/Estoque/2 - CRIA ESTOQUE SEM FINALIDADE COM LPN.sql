...
select cod_erp
from mig_estoque 
where cod_erp not in (select p.cod_produto_rel from produto_relacionado p (nolock))--17860 --5007
group by cod_erp 
...
--begin tran
--update migraestoque set lote = '2002302' where cod_erp = 690716 and endereco = 'PS015203'
--update migraestoque set lote = '11110320' where cod_erp = 678287 and endereco = 'PS025303'
--update migraestoque set lote = '639445' where cod_erp = 701696 and endereco = 'PS025308'
...
select p.cod_produto, ecf.cod_erp , ecf.Estoque, ecf.lote, Convert(SMALLDATETIME, ecf.vencimento, 103), 
Convert(SMALLDATETIME, ecf.fabricacao, 103), endereco as endereco
from produto p (nolock)
inner join produto_relacionado pr (nolock) on p.cod_produto = pr.cod_produto
INNER join mig_estoque ecf on ecf.cod_erp  = pr.cod_produto_rel
--where [Estoque] > 0
--order by 7
...
getdate() as data_cadastro, null as emb_compra, min(CONVERT(datetime, l.DATA_FABRICACAO,105)) as data_fabricacao, null as laudo, 0, 1
FROM Ee L
...
select * from mig_estoque where cod_erp = 400100 