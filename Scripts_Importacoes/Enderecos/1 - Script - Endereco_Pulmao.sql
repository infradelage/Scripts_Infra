...
SELECT 1, 7, 'APTO', 2, 0, NULL, NULL, NULL, 1, 0 
...
select * from mig_pulmao_trabalho
...
update mig_pulmao_trabalho set tipo_de_armazenamento = 'PULMAO' where tipo_de_acesso like '%PULMÃO%'
--FIM AJUSTES
...
--
DELETE endereco where id_def > 5
...
select distinct 1, 2,  m1.esteira, m2.id_endereco, null, m1.esteira, dlc.id_lote_classificacao,1
...
AND NOT EXISTS (SELECT 1 FROM ENDERECO E WHERE E.endereco_completo = m1.esteira AND ID_DEF = 1 AND NIVEL = 2)
and m2.id_def = 1
and m2.nivel = 1

--NIVEL 3 - XX
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco)
select distinct  1, 3, m1.rua, m2.id_endereco, null, m1.rua ,dlc.id_lote_classificacao,1
from mig_pulmao_trabalho m1, endereco m2, def_lote_classificacao dlc
where m2.endereco_completo = m1.esteira
and m1.classificacao_endereco = dlc.descricao
AND NOT EXISTS (SELECT 1 FROM ENDERECO E WHERE E.endereco_completo = m1.rua AND ID_DEF = 1 AND NIVEL = 3)
and m2.id_def = 1
and m2.nivel = 2
...
select distinct 1, 4, m1.modulo, m2.id_endereco, null, m1.rua + m1.modulo, dlc.id_lote_classificacao,1,
NULL, NULL, dtve.id_tipo_volume_endereco
...
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco,
id_tipo_acesso_endereco, id_tipo_Armazenamento, id_tipo_volume)
select distinct 1, 5, m1.nivel, m2.id_endereco, null, m1.rua + m1.modulo + m1.nivel, 
...
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco,
id_tipo_acesso_endereco, id_tipo_Armazenamento, id_tipo_volume)
select distinct 1, 6, m1.apartamento, m2.id_endereco, null, m1.rua + m1.modulo + m1.nivel + m1.apartamento, 
