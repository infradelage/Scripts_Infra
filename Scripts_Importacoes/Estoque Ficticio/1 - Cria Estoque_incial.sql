...
insert ee(cod_produto, codigo_erp, estoque, lote, data_validade, data_fabricacao, endereco)
select p.cod_produto, ecf.cod_produto , ecf.saldo, ecf.lote, Convert(SMALLDATETIME, ecf.dtvalidade, 103), 
Convert(SMALLDATETIME, ecf.dtfabricacao, 103), endereco_completo as endereco --into #ee-- into #E  ],
...
INNER join migraestoque ecf on ecf.cod_produto  = pr.cod_produto_rel
--where [Estoque] > 0
--order by 7
...
getdate() as data_cadastro, null as emb_compra, min(CONVERT(datetime, l.DATA_FABRICACAO,105)) as data_fabricacao, null as laudo
FROM Ee L
