...
SELECT 2, 2, 'LINHA', 3, 1, NULL, NULL, '.', 1, 0 UNION
SELECT 2, 3, 'MODULO', 3, 0, NULL,NULL, '.', 1, 0 UNION
--SELECT 2, 4, 'LADO', 3, 0, NULL,NULL, NULL, 0, 0 UNION
SELECT 2, 4, 'NIVEL',   3, 0, NULL, NULL,NULL, 1, 0 UNION
SELECT 2, 5, 'POSICAO', 3, 0, NULL, NULL, NULL, 1, 0 
...
Exemplo abaixo:
alter table mig_flowrack_trabalho_2 alter column estacao varchar(30)
*/
...
select distinct tipo_de_acesso,3 from mig_flowrack_trabalho_2 
...
update mig_flowrack_trabalho set tipo_volume =  'FLOWRACK ' + substring(tipo_volume,9,2)
update mig_flowrack_trabalho set tipo_volume =  substring(tipo_volume,1,8)+' 0'+ substring(tipo_volume,10,1)
...
--NIVEL 3 - XX
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco)
select distinct  2, 3, m1.MODULO, m2.id_endereco, null, m1.LINHA + '.' + m1.MODULO ,dlc.id_lote_classificacao,1
from mig_flowrack_trabalho m1, endereco m2, def_lote_classificacao dlc
where m2.endereco_completo = m1.LINHA
and m1.classificacao = dlc.descricao
AND NOT EXISTS (SELECT 1 FROM ENDERECO E WHERE E.endereco_completo = m1.LINHA + '.' + m1.MODULO AND ID_DEF = 2 AND NIVEL = 3)
and m2.id_def = 2
and m2.nivel = 2

select * from endereco where id_def = 2 and nivel = 3
...
select distinct 2, 4, m1.NIVEL, m2.id_endereco, null, m1.LINHA + '.' + m1.MODULO + '.' + m1.NIVEL, dlc.id_lote_classificacao,1,
NULL, NULL, null--, m2.endereco_completo
--select m1.LINHA + '.' + m1.MODULO, m1.LINHA + '.' + m1.MODULO + '.' + m1.NIVEl
from mig_flowrack_trabalho m1, endereco m2, endereco m3, def_lote_classificacao dlc 
where m2.endereco_completo = m1.LINHA + '.' + m1.MODULO-- + '.'
and m2.id_pai = M3.id_endereco
and M3.endereco = m1.LINHA
...
--                   where m11.endereco_completo = m1.LINHA + '.' + m1.MODULO + '.' + m1.NIVEl
...
and not exists(select 1 from endereco m11
				...
				and endereco_completo = m1.LINHA + '.' + m1.MODULO + '.' + m1.NIVEL) 
...
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco,
id_tipo_acesso_endereco, id_tipo_Armazenamento, id_tipo_volume)
select distinct 2, 5, m1.POSICAO, m2.id_endereco, null, m1.LINHA + '.' + m1.MODULO + '.' + m1.NIVEL + m1.POSICAO, 
...
where m2.endereco_completo = m1.LINHA + '.' + m1.MODULO + '.' + m1.NIVEL
...
and not exists(select 1 from endereco m11, endereco m55, endereco m22, endereco m33, endereco m44
                   where m11.endereco_completo = m1.LINHA + '.' + m1.MODULO + '.' + m1.NIVEL + m1.POSICAO
...
--insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco,
--id_tipo_acesso_endereco, id_tipo_Armazenamento, id_tipo_volume)
--select distinct 2, 6, m1.POSICAO, m2.id_endereco, null, m1.LINHA + '.' + m1.MODULO + '.' + m1.LADO + m1.NIVEL+ m1.POSICAO, 
--dlc.id_lote_classificacao,1,dtac.id_tipo_acesso_endereco, dtae.id_tipo_armazenamento_endereco,null--dtve.id_tipo_volume_endereco
...
--def_tipo_volume_endereco dtve, def_tipo_armazenamento_endereco dtae
--where m2.endereco_completo = m1.LINHA + '.' + m1.MODULO + '.' + m1.LADO + m1.NIVEL
--and m2.id_pai = m4.id_endereco
...
--                   where m11.endereco_completo = m1.LINHA + '.' + m1.MODULO + '.' + m1.LADO + m1.NIVEL+ m1.POSICAO
...
----select e.*
--from endereco e
--inner join mig_flowrack_trabalho_2 ee on e.endereco_completo = ee.endereco_completo
--where e.id_def = 2
