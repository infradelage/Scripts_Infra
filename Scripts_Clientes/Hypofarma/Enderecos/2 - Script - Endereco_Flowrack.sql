...
select distinct setor1,3 from mig_flowrack_trabalho
where setor1 not in (select descricao from def_endereco_setor where descricao = setor and id_tipo = 3)
...
--NIVEL 3 - XX
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco)
select distinct  2, 3, m1.SETOR, m2.id_endereco, null, m1.SETOR ,dlc.id_lote_classificacao,1
from mig_flowrack_trabalho m1, endereco m2, def_lote_classificacao dlc
where m2.endereco_completo = m1.OL
and m1.classificacao = dlc.descricao
AND NOT EXISTS (SELECT 1 FROM ENDERECO E WHERE E.endereco_completo = m1.SETOR AND ID_DEF = 2 AND NIVEL = 3)
and m2.id_def = 2
and m2.nivel = 2
...
inner join def_endereco_setor def on def.descricao = m1.setor1
