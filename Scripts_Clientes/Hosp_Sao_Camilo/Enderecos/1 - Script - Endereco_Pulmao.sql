...
SELECT 1, 3,'SETOR', 2, 0, NULL, NULL, NULL,  0, 0	UNION
...
--NIVEL 3 - XX
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco)
select distinct  1, 3, m1.SETOR, m2.id_endereco, null, m1.SETOR ,dlc.id_lote_classificacao,1
from mig_pulmao_trabalho m1, endereco m2, def_lote_classificacao dlc
where m2.endereco_completo = m1.OL
and m1.classificacao_endereco = dlc.descricao
AND NOT EXISTS (SELECT 1 FROM ENDERECO E WHERE E.endereco_completo = m1.SETOR AND ID_DEF = 1 AND NIVEL = 3)
and m2.id_def = 1
and m2.nivel = 2
...
select distinct 1, 4, m1.RUA, m2.id_endereco, null, m1.SETOR+m1.RUA, dlc.id_lote_classificacao,1,
...
and not exists(select 1 from endereco m11,  endereco m22, endereco m33, endereco m44
                   where m11.endereco_completo = m1.SETOR + m1.RUA + '.'
...
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco,
id_tipo_acesso_endereco, id_tipo_Armazenamento, id_tipo_volume)
select distinct 1, 5, m1.LADO, m2.id_endereco, null, m1.SETOR+m1.RUA+'.'+m1.LADO, 
...
and m1.classificacao_endereco = dlc.descricao
--AND m1.SETOR+m1.separador+m1.RUA+m1.separador NOT IN ('SND_MAT.A .')
...
and not exists(select 1 from endereco m11, endereco m55, endereco m22, endereco m33, endereco m44
                   where m11.endereco_completo = m1.SETOR + m1.RUA + '.' + m1.LADO + '.'
...
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco,
id_tipo_acesso_endereco, id_tipo_Armazenamento, id_tipo_volume)
select distinct 1, 6, m1.PREDIO, m2.id_endereco, null, m1.SETOR + m1.RUA + '.' + m1.LADO + m1.PREDIO, 
dlc.id_lote_classificacao,1,null, null, null
...
and not exists(select 1 from endereco m11,endereco m12, endereco m55, endereco m22, endereco m33, endereco m44
                   where m11.endereco_completo = m1.SETOR + m1.RUA + '.' + m1.LADO + m1.PREDIO
...
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco,
id_tipo_acesso_endereco, id_tipo_Armazenamento, id_tipo_volume)
select distinct 1, 7, m1.NIVEL, m2.id_endereco, null, m1.RUA + '.' + m1.PREDIO+ '.' + m1.NIVEL,--m1.SETOR + m1.RUA + '.' + m1.LADO  + m1.PREDIO+ '.' + m1.NIVEL,
