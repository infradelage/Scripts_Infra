...
select cod_produto,cod_produto / 10, descricao
from mig_estoque 
where   cod_produto / 10 not in (select p.cod_produto  from produto_relacionado p (nolock))--17860 --5007
--where   cod_produto  not in (select p.cod_produto  from produto_relacionado p (nolock))--17860 --5007
group by cod_produto,descricao

SELECT * FROM mig_estoque where cod_produto = '23361'
select*  from produto_relacionado where cod_produto = '23361'
select *  from produto where (cod_produto) = (23361 /10)
...
YEAR(convert(date, data_fabricacao,105)) ano,
month(convert(date, data_fabricacao,105)) mes,
day(convert(date, data_fabricacao,105)) dia
from mig_estoque
...
YEAR(convert(date, data_vencimento,105)) ano,
month(convert(date, data_vencimento,105)) mes,
day(convert(date, data_vencimento,105)) dia
from mig_estoque
...
insert ee(cod_produto, cod_erp, estoque, lote, data_validade, data_fabricacao, endereco)
select p.cod_produto, ecf.cod_produto , ecf.Estoque, ecf.lote, Convert(SMALLDATETIME, ecf.data_vencimento, 103), 
Convert(SMALLDATETIME, ecf.data_fabricacao, 103), ecf.endereco as endereco --into #ee-- into #E  ],
...
INNER join mig_estoque ecf on ecf.cod_produto / 10  = pr.cod_produto
--where [Estoque] > 0
--order by 7
...
getdate() as data_cadastro, null as emb_compra, min(CONVERT(datetime, l.DATA_FABRICACAO,105)) as data_fabricacao, 1,1, null,null,0 as laudo
FROM Ee L
