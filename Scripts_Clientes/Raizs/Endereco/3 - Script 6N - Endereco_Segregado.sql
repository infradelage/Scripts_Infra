...
SELECT 5, 2, 'SETOR', 6, 0, NULL, NULL, NULL, 0, 0 UNION
SELECT 5, 3, 'RUA', 6, 0, NULL, NULL, '-', 1, 0 UNION
SELECT 5, 4, 'PREDIO', 6, 0, NULL, NULL, '-', 1, 0 UNION
SELECT 5, 5, 'NIVEl', 6, 0, NULL, NULL, '-', 1, 0 UNION
SELECT 5, 6, 'POSICAO', 6, 0, NULL, NULL, NULL, 1, 0 
...
update mig_segregado_trabalho set setor = 'DEVOLUCAO CLIENTE' where classificacao_endereco like '%DEVOLUÇÃO CLIENTE%'
update mig_segregado_trabalho set setor = 'DEVOLUCAO FORN' where classificacao_endereco like '%DEVOLUÇÃO FORN%'
...
update mig_segregado_trabalho set classificacao = 'SEGREGADO' where classificacao like '%SEGREGADOS%'
--FIM AJUSTES
...
select distinct setor2,2 from mig_segregado_trabalho
where setor2 not in (select descricao from def_endereco_setor where descricao = setor and id_tipo = 6)
...
select distinct 5, 2,  m1.SETOR, m2.id_endereco, null, m1.SETOR, dlc.id_lote_classificacao,1
...
AND NOT EXISTS (SELECT 1 FROM ENDERECO E WHERE E.endereco_completo = m1.SETOR AND ID_DEF = 5 AND NIVEL = 2)
and m2.id_def = 5
and m2.nivel = 1

--NIVEL 3 - XX
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco)
select distinct 5, 3, m1.RUA, m2.id_endereco, null, m1.RUA ,dlc.id_lote_classificacao,1
from mig_segregado_trabalho m1, endereco m2, def_lote_classificacao dlc
where m2.endereco_completo = m1.SETOR
and m1.classificacao_endereco = dlc.descricao
AND NOT EXISTS (SELECT 1 FROM ENDERECO E WHERE E.endereco_completo = m1.RUA AND ID_DEF = 5 AND NIVEL = 3)
and m2.id_def = 5
and m2.nivel = 2
...
select distinct 5, 4, m1.PREDIO, m2.id_endereco, null, m1.RUA+'-'+m1.PREDIO, dlc.id_lote_classificacao,1,null,null,null
...
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco,
id_tipo_acesso_endereco, id_tipo_Armazenamento, id_tipo_volume)
select distinct 5, 5, m1.NIVEl, m2.id_endereco, null, m1.RUA+'-'+m1.PREDIO+'-'+m1.NIVEl, 
...
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco,
id_tipo_acesso_endereco, id_tipo_Armazenamento, id_tipo_volume)
select distinct 5, 6, m1.POSICAO, m2.id_endereco, null,m1.RUA+'-'+m1.PREDIO+'-'+m1.NIVEl+'-'+m1.POSICAO, 
dlc.id_lote_classificacao,1,NULL, NULL,NULL
from mig_segregado_trabalho m1, endereco m2, endereco m3, endereco m5, endereco m4, def_lote_classificacao dlc,
...
where e.id_def = 5 and def.id_tipo = 4--SEGREGADO
