...
SELECT 4, 3, 'CONF', 5, 0, NULL, NULL, '-', 1, 0 UNION
SELECT 4, 4, 'PALETE', 5, 0, NULL, NULL, NULL, 1, 0

ALTER TABLE endereco CHECK CONSTRAINT ALL
GO

--Valida a def_endereco criada
...
update mig_doca_trabalho set classificacao_endereco = 'NORMAL'

select distinct tipo_volume from mig_doca_trabalho
update mig_doca_trabalho set tipo_volume = 'PALLET PADRAO' where tipo_volume = 'PALATE PADRÃO'
...
select distinct  4, 3, m1.CONF, m2.id_endereco, null, m1.doca + '-' + m1.CONF ,dlc.id_lote_classificacao,1,
...
AND NOT EXISTS (SELECT 1 FROM ENDERECO E WHERE E.endereco_completo = m1.doca + '-' + m1.CONF AND id_def = 4 AND NIVEL = 3)
and m2.id_def = 4
and m2.nivel = 2
...
insert endereco(id_def, nivel, endereco, id_pai, qtd_caixa, endereco_completo, id_lote_classificacao, cod_situacao_endereco,
id_tipo_acesso_endereco, id_tipo_Armazenamento, id_tipo_volume)
select distinct  4, 4, m1.PALETE, m2.id_endereco, null, m1.DOCA + '-' + m1.CONF + '-' + m1.PALETE ,dlc.id_lote_classificacao,1,
dtac.id_tipo_acesso_endereco, dtae.id_tipo_armazenamento_endereco,dtve.id_tipo_volume_endereco
from mig_doca_trabalho m1, endereco m2, def_lote_classificacao dlc, def_tipo_acesso_endereco dtac, def_tipo_volume_endereco dtve, 
def_tipo_armazenamento_endereco dtae
where m2.endereco_completo = m1.DOCA + '-' + m1.CONF
and m1.classificacao_endereco = dlc.descricao
and dtac.id_tipo_endereco = 5 -- STAGING
and dtve.descricao =  m1.tipo_volume
and dtae.descricao = m1.tipo_de_armazenamento
AND NOT EXISTS (SELECT 1 FROM ENDERECO E WHERE E.endereco_completo = m1.DOCA + '-' + m1.CONF + '-' + m1.PALETE AND id_def = 4 AND NIVEL = 4)
and m2.id_def = 4
and m2.nivel = 3
...
where e.id_def = 4 and def.id_tipo = 5--STAGING/DOCA
...
Validacoes
 ...
 SELECT ENDERECO_COMPLETO, COUNT(1) as Duplicidades FROM mig_doca_trabalho GROUP BY ENDERECO_COMPLETO HAVING COUNT(1) > 1
