...
--begin tran
--update migraestoque set lote = '2002302' where cod_produto = 690716 and endereco = 'PS015203'
--update migraestoque set lote = '11110320' where cod_produto = 678287 and endereco = 'PS025303'
--update migraestoque set lote = '639445' where cod_produto = 701696 and endereco = 'PS025308'
...
update ecf set vencimento = '02/03/2023'
--select *
from mig_estoque ecf
where substring(vencimento,7,5) > '2032'
GO

IF(OBJECT_ID('tempdb..#ee') IS NOT NULL)
    DROP TABLE #ee
create table #ee (id_tabela int identity (2,1), cod_produto int, cod_produto_rel varchar (30), estoque money, lote varchar(30), 
data_validade smalldatetime, data_fabricacao smalldatetime, endereco varchar(30), numero varchar(20))

insert #ee(cod_produto, cod_produto_rel, estoque, lote, data_validade, data_fabricacao, endereco, numero)
select p.cod_produto, ecf.cod_produto , ecf.Estoque, ecf.lote, Convert(SMALLDATETIME, ecf.vencimento, 103), 
Convert(SMALLDATETIME, ecf.fabricacao, 103), endereco as endereco, lpn
from produto p (nolock)
inner join produto_relacionado pr (nolock) on p.cod_produto = pr.cod_produto
INNER join mig_estoque ecf on RTRIM(ecf.cod_produto)  = pr.cod_produto_rel
--where [Estoque] > 0
--order by 7


--select substring(vencimento,7,5)-- Convert(SMALLDATETIME, ecf.vencimento, 103) 
--from mig_estoque ecf
--order by  substring(vencimento,7,5) desc

--select *
--from mig_estoque ecf
--where substring(vencimento,7,5) > '2032'

--update ecf set vencimento = '02/03/2024'
----select vencimento
--from mig_estoque ecf
--where substring(vencimento,7,5)>= '2108'
...
--DELETE isaldo_validade_220917 WHERE convert(varchar(20),cod_produto) not in (select cod_produto_rel from produto_relacionado)

--select * from #E where cod_produto= 454
--select * from lote_estoque_lpn where id_lote = 3
--select * from lote where cod_produto= 454
--select * from isaldo_validade_220917 where cod_produto = 220834
...
--DELETE lote DBCC CHECKIDENT('[lote]', RES#eeD, 1)
--begin tran


--insert def_lote_classificacao_finalidade (descricao, padrao,permite_alteracao, rastreavel, abreviacao)
--select distinct finalidade, 0, 1, 0 , null 
--from mig_estoque 
--where finalidade not in (select descricao from def_lote_classificacao_finalidade)

--PASS0 4
---DEFINITIVO---
ALTER TABLE lote DISABLE TRIGGER ALL
INSERT lote
SELECT L.cod_produto, L.LOTE, min(CONVERT(datetime, l.vencimento,105)) as validade, 1 as cod_situacao, 
getdate() as data_cadastro, null as emb_compra, min(CONVERT(datetime, l.fabricacao,105)) as data_fabricacao, 0 as lacre, def.id_lote_classificacao_finalidade, null,null,null
FROM mig_estoque L
inner join def_lote_classificacao_finalidade def on l.finalidade = def.descricao
...
GROUP BY L.cod_produto, L.LOTE, def.id_lote_classificacao_finalidade
...
GROUP BY MIGRA.numero,e.id_endereco
ORDER BY 1
...
--select * from vw_lote_estoque where cod_produto = 400100

--select * from mig_estoque where cod_produto = 400100 
