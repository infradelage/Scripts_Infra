...
SET @cod_pedido = (Select max(cod_pedido) + 1 from pedido)

set IDENTITY_INSERT pedido on
insert pedido (cod_pedido,cod_entidade,operacao,cod_situacao,digitacao,termino_digitacao,data_liberacao,encerramento_PN,faturamento,quem_liberou,quem_digitou,cod_origem_ped,cod_operacao_logistica,obs_PN,data_exportacao)
values (@cod_pedido,1, 2, 4, getdate(), getdate(), getdate(), getdate(), getdate(), 'delage','delage', 1, 1, 'Migração inicial de estoque', getdate()  )
