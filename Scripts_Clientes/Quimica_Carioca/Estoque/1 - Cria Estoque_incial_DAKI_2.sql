...
--alter table migraestoque2 add data_validade smalldatetime
--alter table migraestoque2 add data_fabricacao smalldatetime

update migraestoque2 set DATA_VALIDADE = '2025-12-31', data_fabricacao = dateadd(d, -1, getdate())
...
select * from migraestoque2 where cod_produto_polo = '11762'
...
--update migraestoque2 set data_de_vencimento = '2021/02/28 00:00' where  data_de_vencimento = '2021/02/29 00:00'

--select (CONVERT(datetime, substring(data_de_fabricacao,1,11),121)) 
--from migraestoque2 l
...
--drop table migraestoque2


select* from migraestoque2 order by data_de_vencimento asc
select* from migraestoque2 order by data_De_fabricacao asc

--begin tran
--update migraestoque2 set lote = '2002302' where cod_produto_polo = 690716 and endereco = 'PS015203'
--update migraestoque2 set lote = '11110320' where cod_produto_polo = 678287 and endereco = 'PS025303'
--update migraestoque2 set lote = '639445' where cod_produto_polo = 701696 and endereco = 'PS025308'
--update migraestoque2 set endereco = replace(endereco,'.','')
--update migraestoque2 set lote = replace(lote,'.','')
--commit tran
...
insert ee(cod_produto, cod_produto_polo, estoque, lote, data_validade, data_fabricacao, endereco, numero)
--select p.cod_produto, cod_produto_polo , [Estoque], ecf.lote_validade, Convert(SMALLDATETIME, ecf.lote_validade, 103), 
...
--INNER join migraestoque2 ecf on ecf.cod_produto_polo  = pr.cod_produto_rel
--where ecf.estoque>0
--where [Estoque] > 0
--order by 7
select pr.cod_produto, pr.cod_produto_rel, le.estoque, l.lote, l.validade, l.data_fabricacao, e.endereco_completo, lpn.numero
 FROM lote_estoque_lpn LE(nolock) inner join    
       lpn (NOLOCK) ON LPN.id_LPN = LE.id_LPN inner join  
       lote L (NOLOCK) ON LE.id_lote = L.id_lote INNER JOIN  
       endereco E (NOLOCK) ON LPN.id_endereco = E.id_endereco INNER JOIN
	   produto_relacionado pr (nolock) on l.cod_produto = pr.cod_produto
where e.id_def in (1,2)



select * into ee from WMSRXDaki01..ee
...
getdate() as data_cadastro, null as emb_compra, min(CONVERT(datetime, l.DATA_FABRICACAO,105)) as data_fabricacao, 0, 1, null, null
FROM Ee L
...
getdate() as data_cadastro
  ...
  INNER JOIN endereco AS e WITH(NOLOCK) ON MIGRA.ENDERECO = e.id_endereco --and e.id_def = 1
